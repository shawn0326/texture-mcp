...
项目目标：构建一个通过 MCP 暴露的程序化贴图生成工具，将高层语义 preset 转换为可执行 recipe，在不依赖生成式图像模型的前提下，实现可控、可复现、可由 AI 调用的贴图生成能力。

## 当前状态

- 当前项目的 MVP 主线已完成，可通过 MCP 调用生成、导出贴图，并提供 preset 信息查询。
...
- 新图元应尽量保持单一职责；复杂组合优先通过 `group` 或 preset 组织，而不是让单个图元承担过多语义。
